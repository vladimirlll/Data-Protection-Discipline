...
namespace DI.Lab2.Services.Models
{
    /// <summary>
    ...
    public class ScramblerSettings
    {
        ...
        public class Builder
        {
            ...
            public Builder SetKey(IEnumerable<int> key)
            {
                if (_istSet && _isTSet)
                {
                    if (key.Count() == int.Parse(
                        Math.Truncate(
                            _result.T / _result.t).ToString()))
                    {
                        _result.Key = key;
                    }
                    else
                        throw new ArgumentOutOfRangeException("Ключ криптографического преобразования задан неверно. " +
                            $"Количество элементов в списке не равно T / t = {_result.T / _result.t}");
                }
                else
                    throw new InvalidOperationException("Не задан малый или большой временной интервал");
